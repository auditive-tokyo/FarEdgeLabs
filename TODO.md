...
> 無音45秒 / 通話3分で切る（`use-realtime-call.ts`）。**GPT-Live は「セッションが
> 開いている時間」で課金される**ので、タブを開いたまま放置されると止まらない。
> Realtime とは逆の性質（あちらは server VAD 下の無音が 0 トークン）で、**載せ替えた
> 時点で危険度が変わっていた**。1晩で $24、1週間で $500。
>
