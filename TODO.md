...
| 手 | 効果 | いつ入れるか |
|---|---|---|
| **`gpt-live-1` から降りる** | 音声層が $0.05/分。`gpt-realtime-2.1-mini` なら実効 $0.027/分 | 全二重と発音が要らないと判断できたら。**戻すと長話も戻る**ので、`instruction.py` の分離を先に確認すること |

> [!note] 時間切れタイマーは入れた（2026-09-15）
> 無音45秒 / 通話3分で切る（`use-realtime-call.ts`）。
>
> **壁時計で課金されることは実測した。** マイクを切って放置し、残高が
> **$1.03 → $1.21（3.6分ぶん、$0.05/分と一致）**。ドキュメントの
> "billed by duration, per second" は「音声のやり取りがある時間」ではなく
> **セッションが開いている時間**だった。Realtime とは逆の性質（あちらは server VAD
> 下の無音が 0 トークン）で、**載せ替えた時点で危険度が変わっていた**。
> 放置すれば1晩 $24、1週間 $500 —— **ただし OpenAI 側の月次上限を $30 にしてあるので、
> 最悪でもそこで止まる。** ブラウザ側のタイマーは DevTools を開ける相手には効かないので、
> 本当の上限はあちら。
>
> **無音タイマーは最初に入れた版が動いていなかった。** 「どのイベントでも叩き直す」
> にしていたが、`session.usage.updated` が定期的に流れていて延々とリセットしていた。
> 除外リストに直した。**推測でイベント名を選ばず、`npm run dev` のコンソールで実物を
> 見てから書くこと** —— この関数では `file_search` でも同じ形の失敗をしている。
>
> `max_output_tokens` の行は消した。あれは Realtime の出力トークン課金に効く手で、
> **時間課金の GPT-Live には効かない。**
...
>
> 事故対策のほうは、**ブラウザ側のタイマーがインフラゼロで**達成している（上の note）。
