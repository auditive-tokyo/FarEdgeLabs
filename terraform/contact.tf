...
# > ブラウザが直接叩くので `allUsers` に `run.invoker` を渡す（下の方）。前段に認証が
# > 無いので、防御は `main.py` の4層と、ここの `max_instance_count` に
# > しかない。
...
# > 動かなくなる。** 同じ理由で無認証なのが `realtime.tf` の `realtime-call` で、
# > いまこのプロジェクトの関数はその2本だけ。どちらも `allUsers` が正しい。
# >
