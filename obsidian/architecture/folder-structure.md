...
├── scripts/                 ← build-time asset generation (`yarn brand`)
...
│   └── fonts/              # Self-hosted faces for next/font/local (General Sans)
│                           #   (no favicon.ico here — it would shadow public/'s,
│                           #    see [[seo-metadata]])
│
├── views/                  # Page-level components — one per route
│   ├── home.tsx            # HomeView (Server Component) — assembles the page
│   └── home/               # ...and its own sections, next to it, not in components/
...
│       └── hero-stats.tsx · hero-request.tsx · hero-trust.tsx
│
...
│   ├── common/             # Shared infrastructure — Cookie, grid, Preloader,
│   │                       #   HalftoneVideo, ReducedMotion, Skeletons
...
│                            # ALL generated by `yarn brand` — don't hand-edit.
...
| A favicon / icon / OG / manifest asset | `public/` root — but generate it, see [[seo-metadata]] |
| A build-time asset generator | `scripts/*.mjs` + a `package.json` script |
